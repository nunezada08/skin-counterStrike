...
import { RARITIES, MOCK_SKINS } from '../data/mockSkins'

const RARITY_MAP = Object.fromEntries(RARITIES.map(r => [r.value, r]))

function pickWeightedSkin() {
  // Weight by rarity chance
  const pool = []
  MOCK_SKINS.forEach(skin => {
...
function buildReel(winnerSkin, totalItems = 60) {
  // Build a long strip; winner is at a fixed position near the end
  const WINNER_IDX = totalItems - 8
  const items = []
  for (let i = 0; i < totalItems; i++) {
    items.push(i === WINNER_IDX ? winnerSkin : pickWeightedSkin())
  }
...
export default function Roulette() {
  ...
  function spin() {
    ...
    const winnerSkin = pickWeightedSkin()
    const { items, winnerIdx } = buildReel(winnerSkin)
    setReel(items)
    setSpinning(true)

    // Let React render the reel, then animate
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (!trackRef.current) return
        const viewportCenter = 320 // half of the ~640px container
        ...
        <div className="relative bg-[#0d0d14] border border-white/10 rounded-xl h-40 overflow-hidden">
          ...
          {reel ? (
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-600 text-sm">Pressione girar para começar</p>
            </div>
          )}
        </div>
      ...
      {winner && showWinner && (
        ...
        >
          ...
          <div className="flex items-center justify-center gap-4 text-sm">
            ...
            <span className="text-[#f0a500] font-bold">${winner.price.toFixed(2)}</span>
            <span className="text-gray-500">·</span>
