...
function preloadImages(imageUrls) {
    const promises = imageUrls.map((url) => {
        return new Promise((resolve) => {
            ...
            img.src = url;
            img.onload = () => resolve();
            img.onerror = () => resolve(); // Resolve mesmo em erro para não travar a roleta
        });
    });
...
export default function Roulette() {
    ...
    const [preloading, setPreloading] = useState(false); // Novo estado para pré-carregamento
    ...
    async function spin() {
        ...
        // Atualiza o reel primeiro para renderizar os elementos no DOM
        setReel(items);
        setPreloading(true);

        // FIX: Pré-carregar todas as imagens do reel antes de começar a girar
        // Pegamos apenas as URLs únicas para ser mais eficiente
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                // Resetar posição e transition
                trackRef.current.style.transition = 'none';
                trackRef.current.style.transform = 'translateX(0px)';

                // Cálculo do targetX corrigido (Bug 2 anterior)
        ...
        <div className="max-w-3xl mx-auto px-4 py-10 space-y-10">
            <div className="text-center space-y-2">
                ...
                <p className="text-gray-500 text-sm max-w-md mx-auto">
                    Gire a roleta e veja qual skin você teria desbloqueado. Cada raridade tem uma
                    chance diferente de aparecer.
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {RARITIES.map((r) => (
                    <div
                        key={r.value}
                        className="bg-[#111118] border border-white/5 rounded-lg p-2.5 text-center">
                        <div
                            className="w-2 h-2 rounded-full mx-auto mb-1.5"
                            style={{ background: r.color }}
                        />
                        <p className="text-[10px] text-gray-400 leading-tight">{r.label}</p>
                        <p className="text-xs font-bold mt-0.5" style={{ color: r.color }}>
                            {r.chance}%
                        </p>
                    </div>
                ))}
            ...
            <div className="space-y-4">
                ...
                <div className="flex gap-3 justify-center">
                    <button
                        onClick={spin}
                        disabled={spinning || loadingSkins || preloading}
                    ...
                    {(winner || reel) && !spinning && !preloading && (
                        <button
                            onClick={reset}
            ...
            {winner && showWinner && (
                <div
                    ...
                    <p
                        ...
                        style={{ color: RARITY_MAP[winner.rarity]?.color }}>
                        🎉 Você desbloqueou
                    </p>
